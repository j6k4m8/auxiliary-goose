...
class YrStoryProvider(StoryProvider):
    def __init__(self, lat: float = 51.477, lon: float = 0.000, F: bool = False, locationName: str = "London"):
        self.F = F
        self.location = Place(locationName,lat,lon)
        self.forecast = Forecast(self.location,"goosepaper/0.3.1 https://github.com/j6k4m8/goosepaper", forecast_type="complete")
    ...
    def get_temps(self, a,b) -> List[float]:
        ...
        for i in range(a,b):
            temps.append(self.forecast.data.intervals[i].variables["air_temperature"].value)
    ...
    def get_precipitation(self, a: int, b: int) -> float:
        ...
        precip = 0
        for i in range(a,b):
            precip += self.forecast.data.intervals[0].variables["precipitation_amount"].value

        return precip

    def get_wind_direction(self, interval: int) -> str:
        
        direction =  self.forecast.data.intervals[interval].variables["wind_from_direction"].value
    ...
    def get_wind(self, interval: int) -> List[float]:
        wind_speed = self.forecast.data.intervals[interval].variables["wind_speed"].value
